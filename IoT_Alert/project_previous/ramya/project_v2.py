...
def EmailClient():
	...
	if State == 'OK':	
		...
		(Typ,BuildFailStringAddress)=Conn.uid('search', '(UNSEEN)','(HEADER Subject "Build failed in Jenkins")','(SENTSINCE {0})'.format(Date))
		if Typ == 'OK':
			ExtractMail(Conn,BuildFailStringAddress,Interaction,Timer,"red","Fail")
		else :
			PopUp("Domain server issue","violet","Fail",Timer)
			sys.exit(1)
		(Typ,BuildPassStringAddress)=Conn.uid('search', '(UNSEEN)','(HEADER Subject "Build Success in Jenkins")','(SENTSINCE {0})'.format(Date))
		if Typ == 'OK':
