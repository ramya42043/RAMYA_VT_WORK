...
def PopUp(Subject,clr,State,Timer=0):
	...
	frame.pack()
	for i in Subject:
		c = Canvas(frame, bg=clr, width=Root.winfo_screenwidth(), height=Root.winfo_screenheight()/5)
		c.pack()
		c.create_text(Root.winfo_screenwidth()/2, Root.winfo_screenheight()/10, text=i,anchor="center", font=('TimesNewRoman', 15))
		if State == "Fail":
			Flashing(Root,clr,c)
