...
def PopUp(Subject,clr,State,Timer=0):
	...
	for i in range(row):
		for j in range(colum):
			...
			canvas = Canvas(top,bg=clr,width=Root.winfo_screenwidth()/(row+1),height=Root.winfo_screenheight()/(colum+1))
			Text=canvas.create_text(Root.winfo_screenwidth()/(row+4),Root.winfo_screenheight()/(colum+4))
