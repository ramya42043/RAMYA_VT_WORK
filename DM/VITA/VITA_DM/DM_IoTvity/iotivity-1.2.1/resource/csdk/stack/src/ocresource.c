...
static OCStackResult HandleVirtualResource (OCServerRequest *request, OCResource* resource)
{
...
#endif
    {
        ...
        if(discoveryResult == OC_STACK_OK)
        {
            ...
            OCDiscoveryPayload *payload2 = (OCDiscoveryPayload *)payload;

              size_t i = 0;
              OCResourcePayload* p = (OCDiscoveryPayload*) payload2->resources;
              while(p)
              {
                  ++i;
                  p = p->next;
              ...
              //printf("before SendNonPersistantDiscoveryResponse  %d \n",i);

              if(i > 4)
                    SendNonPersistantDiscoveryResponse(request, resource , payload, OC_EH_OK);

            //printf("after SendNonPersistantDiscoveryResponse\n");
...
void non_iotvityAddresource(char *resName) {
//    printf("non_iotivity_test ========================= \n");
//    printf("non_iotivity_test ======== %s\n ",resName);

    OCServerRequest * testRequest = (OCServerRequest *)OICCalloc(1, sizeof(OCServerRequest));;
    testRequest->method = 1;
    testRequest->acceptFormat=0;
    strcpy(testRequest->resourceUrl, "/oic/res/");
    testRequest->qos= 0;
    testRequest->observationOption=2;
    testRequest->observeResult=255;
    testRequest->numResponses=1;
    testRequest->tokenLength=8;
    testRequest->delayedResNeeded=0;
    testRequest->numRcvdVendorSpecificHeaderOptions=0;
    testRequest->requestComplete=1;
    testRequest->slowFlag=0;
    testRequest->payloadSize=0;
    testRequest->payload[1]=0;
    testRequest->requestId=1234567;

    OCResource * testresource = (OCResource *)OICCalloc(1, sizeof(OCResource));;


    OCPayload *Payload1 = NULL;

    discoveryPayloadCreateAndAddDeviceId(&Payload1);
    OCDiscoveryPayload *discPayload1 = (OCDiscoveryPayload *)Payload1;



    OCResource *resource1 = (OCResource *)OICCalloc(1, sizeof(OCResource));;

    resource1->uri = OICStrdup(resName);


     resource1->rsrcType = (OCResourceType *)OICCalloc(1, sizeof(OCResourceType));
     resource1->rsrcType->resourcetypename = OICStrdup("core.light");

     OCResourceType* typePtr = (OCResourceType *)OICCalloc(1, sizeof(OCResourceType));
     typePtr->resourcetypename = OICStrdup("core.brightlight");

     typePtr->next = NULL;
     resource1->rsrcType->next = typePtr;



     resource1->rsrcInterface = (OCResourceInterface *)OICCalloc(1, sizeof(OCResourceInterface));
     resource1->rsrcInterface->name = OICStrdup("oic.if.baseline");

     OCResourceInterface* typePtr1 = (OCResourceInterface *)OICCalloc(1, sizeof(OCResourceInterface));
     typePtr1->name = OICStrdup("oic.if.ll");
     typePtr1->next = NULL;
     resource1->rsrcInterface->next = typePtr1;



    BuildVirtualResourceResponse(resource1, discPayload1, NULL);
    SendNonPersistantDiscoveryResponse(testRequest, testresource, Payload1, OC_EH_OK);
