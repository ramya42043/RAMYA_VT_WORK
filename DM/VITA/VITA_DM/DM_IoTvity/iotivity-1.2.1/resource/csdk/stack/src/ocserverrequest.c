...
//OCStackResult non_iotvityGetdata(char *uriname);

OCStackResult   non_iotvityGetdata(char *uriname, key_value_pair_t1 *data)
{
//  /  return 101 ;

    //printf("non_iotvityGetdata ----- 1\n ");


        data = NI_testGetdata;
        //printf("address of data in get method = %.8x \n",data);
       // printf("address of data next in get method = %.8x \n",data->pNext);

//     printf("calling non_iotvityGetdata\n ");
//        OCEntityHandlerResponse *ehResponse = (OCEntityHandlerResponse *)OICCalloc(1, sizeof(OCEntityHandlerResponse));
//        ehResponse->requestHandle = 1234567;
//        ehResponse->ehResult = 0;
//        ehResponse -> numSendVendorSpecificHeaderOptions = 0;
//        ehResponse -> persistentBufferFlag = 0;
//        strcpy(ehResponse -> resourceUri,"non_iotvity-'00:1A:7D:DA:71:13'");

//        ehResponse->payload = NULL;

//        OCRepPayload *repPayload  = (OCRepPayload*)ehResponse->payload;
//        repPayload= (OCRepPayload *)OICCalloc(1, sizeof(OCRepPayload));

//        repPayload->next =NULL;
//        repPayload->uri = OICStrdup("non_iotvity-'00:1A:7D:DA:71:13'");
//        repPayload->base.type = PAYLOAD_TYPE_REPRESENTATION;



//        OCRepPayload *head = NULL;
//        head =  repPayload;
...
//        if(data != NULL)
//        {
//            OCRepPayloadValue *node = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));
//            node->name = OICStrdup(data->key);
//            node->str = OICStrdup("123");
//            node->type = 4;
//            node->next = NULL;
...
////            OCRepPayloadValue *node1 = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));
////            node1->name = OICStrdup("data->pNext->key");
////            node1->str = OICStrdup("data->pNext->->value");
////            node1->type = 4;
////            node1->next = NULL;
...
////            head->values = head->values->next;
////            data = data->pNext;


//            //repPayload ->values = node;
...
//        while (data ->pNext != NULL)
//        {

//            data = data->next;

//            OCRepPayloadValue *repPayload1 = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;
//            repPayload1->type = 4;
//            repPayload1->values->name = OICStrdup(data->key);
//            repPayload1->values->str = OICStrdup(data->value);
//            repPayload1->values->next = NULL;

//             head -> next = repPayload1;
...
//            printf("non_iotvityGetdata ----- 2\n ");


//            printf("non iotvity data %s \n",data->key);
...
//            repPayload->values->next = repPayload1;
...
//        }
    ...
    ehResponse -> numSendVendorSpecificHeaderOptions = 0;
    ehResponse -> persistentBufferFlag = 0;
    strcpy(ehResponse -> resourceUri,uriname);
    ...
    repPayload->values = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;

    repPayload->values->type= 4;
    repPayload->values->name = OICStrdup(data->key);
    repPayload->values->str = OICStrdup(data->value);
    repPayload->values->next = NULL;
    ...
    OCRepPayloadValue *repvalue1 = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;
    repvalue1->type = 4;
    repvalue1->name = OICStrdup(data->pNext->key);
    repvalue1->str = OICStrdup(data->pNext->value);;


    OCRepPayloadValue *repvalue2 = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;
    repvalue2 ->type = 4;
    repvalue2->name = OICStrdup(data->pNext->pNext->key);
    repvalue2->str = OICStrdup(data->pNext->pNext->value);;


    repPayload->values->next = repvalue1;
    repvalue1->next = repvalue2;
    repvalue2->next = NULL;
#endif

    repPayload->values = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;

    OCRepPayloadValue **insert_at = &repPayload->values;

    while(data!=NULL)
    {
        OCRepPayloadValue *repvalue = (OCRepPayloadValue *)OICCalloc(1, sizeof(OCRepPayloadValue));;
        repvalue->type = 4;
        repvalue->name = OICStrdup(data->key);
        repvalue->str = OICStrdup(data->value);
        *insert_at = repvalue;
        insert_at = &repvalue->next;
        data = data->pNext;
    }

    //printf("non_iotvityGetdata ----- 3\n ");



    OCPayload *sample = (OCPayload *)OICCalloc(1, sizeof(OCPayload));;

    sample = (OCPayload *) repPayload;

    ehResponse->payload = sample;
   ...
   OCServerRequest *serverRequest = GetServerRequestUsingHandle(ehResponse->requestHandle);
    ...
    //OCServerRequest *serverRequest = GetServerRequestUsingHandle(1234567);
    if (NULL == serverRequest)
    {
        printf("Error in request code \n");
  ...
  //      OCServerRequest * serverRequest = (OCServerRequest *)OICCalloc(1, sizeof(OCServerRequest));;
        serverRequest->method = 1;
        serverRequest->acceptFormat=OC_FORMAT_CBOR;
        strcpy(serverRequest->resourceUrl,uriname);
        serverRequest->qos= 0;
        serverRequest->observationOption=2;
        serverRequest->observeResult=255;
        serverRequest->numResponses=1;
        serverRequest->tokenLength=8;
        serverRequest->delayedResNeeded=0;
        serverRequest->numRcvdVendorSpecificHeaderOptions=0;
        serverRequest->requestComplete=1;
        serverRequest->slowFlag=0;
        serverRequest->payloadSize=0;
        serverRequest->payload[1]=0;
        serverRequest->requestId=1234567;
        serverRequest->notificationFlag = 0;
        serverRequest->coapID = 1234;
   ...
   responseInfo.info.resourceUri = serverRequest->resourceUrl;
    responseInfo.info.resourceUri = OICStrdup(uriname);

    //printf("HandleSingleResponse == %s\n",responseInfo.info.resourceUri);
...
//    }
   ...
   // responseInfo.info.messageId = serverRequest->coapID;
    ...
    if(responseInfo.info.numOptions > 0)
    {
        ...
        //printf("Test_HandleSingleResponse ======= 2\n");
    ...
    {
       // printf("Test_HandleSingleResponse ======= 3\n");
...
OCStackResult   non_iotvityObservedata(char *uriname, key_value_pair_t1 *data)
{
    ...
    //printf("non_iotvityobservedata ----- 3\n ");
  ...
  //      OCServerRequest * serverRequest = (OCServerRequest *)OICCalloc(1, sizeof(OCServerRequest));;
   ...
   // responseInfo.info.messageId = serverRequest->coapID;
    ...
    responseInfo.info.numOptions = 1;
