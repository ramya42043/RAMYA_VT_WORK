...
static void Handlenon_iotvityCallback(char *uri,unsigned int type,key_value_pair_t1 *data);
...
extern void non_iotvityAddresource(char *);
extern OCStackResult non_iotvityGetdata(char *uri, key_value_pair_t1 *data);

void Handlenon_iotvityCallback(char *uri, unsigned int type, key_value_pair_t1 *data)
{


    if(type == 0)
    {
        //printf("Call discover %d \n",type);

        non_iotvityAddresource(uri);
    }
    else if(type == 1)
    {
       non_iotvityGetdata(uri,data);

    }

    else if(type == 2)
    {
        non_iotvityObservedata(uri,data);
    }

//    if(errorInfo == NULL)
//    {
//        //printf("Calling Non IOTVITY test code %s\n",endPoint->addr);
//        //non_iotivity_test(endPoint->addr);
//        if(strcmp (endPoint->addr,"/a/light"))
//        {
//            nonIotvity_getData();
//        }
//        return;
//    }
    ...
    //printf("Handlenon_iotvityCallback = %s \n",uri);
}
...
void DiscoverySetHandler(void) {
    setNOnIotivityhandler(Handlenon_iotvityCallback);
}
...
OCStackResult OCInit1(OCMode mode, OCTransportFlags serverFlags, OCTransportFlags clientFlags)
{
    ...
    switch (myStackMode)
    {
        case OC_CLIENT:
            ...
            CARegisterNon_iotvityHandler(Handlenon_iotvityCallback);
