...
extern int8_t ADD_ADDRESS(int8_t *);
extern void NON_IOTIVITY_DEVICE(const char*);
extern void add_to_ble_structure(int32_t FD, int8_t * ADDRESS);
    ...
    gpointer                   user_data)
{
    ...
    ///
    /// On properties changed lists the devices currently scannable
    ///
    char * address = g_variant_print(g_dbus_proxy_get_cached_property(interface_proxy,"Address"),TRUE);
    address[18]='\0';
    //printf("\n Address:%s\n",address+1);
//    if(ADD_ADDRESS(address+1) == 1) {
//                        return;
//    }

    char *uuids = g_variant_print(g_dbus_proxy_get_cached_property(interface_proxy,"UUIDs"),TRUE);
    if (strstr(uuids, CA_GATT_SERVICE_UUID) != NULL
            ||
            strstr(uuids,CA_GATT_SERVICE_UUID_LOWER) != NULL)
    {
//        printf("\nIOT Address:%s\n",address+1);
//        add_to_ble_structure(-1, address+1);
    }
    else if(strcasecmp(uuids,"@as []")==0)
    {
//        printf("\nNOn-IOT Address:%s\n",address+1);
        NON_IOTIVITY_DEVICE(address+1);
    }
...
//extern unsigned char* NonIoTivity_AddDevice (unsigned char *ucpMacId,unsigned char *ucpDeviceName);
                                     ...
                                     GVariant * parameters)
{
    ...
    char * address = g_variant_print(g_dbus_proxy_get_cached_property(proxy,"Address"),TRUE);
    address[18]='\0';
//    if(ADD_ADDRESS(address+1) == 1)
//                        return;
    char *uuids = g_variant_print(g_dbus_proxy_get_cached_property(proxy,"UUIDs"),TRUE);
    if (strstr(uuids, CA_GATT_SERVICE_UUID) != NULL
                        ||
        strstr(uuids,CA_GATT_SERVICE_UUID_LOWER) != NULL)
    {
...
void START_DISCOVERY(gpointer proxy, gpointer data) {
    printf("%s\n", __func__);
...
void stopdiscovery(void) {
    printf("%s\n", __func__);
    g_list_foreach(g_context.adapters, START_DISCOVERY, NULL);
}
