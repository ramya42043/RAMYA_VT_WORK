...
extern void add_to_ble_structure(int32_t, int8_t*);
...
bool CACentralConnect(GDBusProxy * device)
{
    ...
    char * address =g_variant_print(g_dbus_proxy_get_cached_property(device, "Address"), TRUE);
        address[18]='\0';
//        char name[100];
//        strcpy(name,g_variant_print(g_dbus_proxy_get_cached_property(device,"Name"),TRUE));
//        if(strstr(g_variant_print(g_dbus_proxy_get_cached_property(device,"Uuid"),TRUE),CA_GATT_SERVICE_UUID)!=NULL)
