...
void NON_IOTIVITY_DEVICE(char* address)
{
...
static void CALEDataReceiverHandler(void *threadData)
{
    ...
    if (g_dataBleReceiverHandlerState)
    {
        ...
        const char *remoteAddress = bleData->remoteEndpoint->addr;//to get address for unicasting
        ...
        if (senderInfo->totalDataLen == senderInfo->recvDataLen)
        {
            ...
            ca_mutex_unlock(g_bleAdapterReqRespCbMutex);
            //Disable connection for the device once the full data is up
            Disable_Connection_Each_Device(remoteAddress);
...
static void CALEClientSendDataThread(void *threadData)
{
    ...
    /*.............................................*/
    char * mydata = (char *)bleData->data;
//    {
//    printf("Ble DATA\n");
//    for (int ble = 0; ble < totalLength; ble++) {
//        printf("0x%.2x(%c)  ", mydata[ble], mydata[ble]);
//    }
//    printf("\n");
//    }
    /*.............................................*/
    ...
    if (NULL != bleData->remoteEndpoint) //Sending Unicast Data
    {
        ...
        // Send the first segment with the header.
        if(mydata[12] == 0x51 && mydata[2] == 0x01) {//to raise observe task
            printf("observe started\n");
            add_to_ble_structure_state(START_OBSERVE, bleData->remoteEndpoint->addr);
        }
        else if(mydata[13] == 0x51 && mydata[2] == 0x01) {//to raise cancel observe task
            printf("cancel observe started\n");
            add_to_ble_structure_state(CANCEL_OBSERVE, bleData->remoteEndpoint->addr);
        }
//        else {
//            add_to_ble_structure_state(DEFAULT, bleData->remoteEndpoint->addr);
//        }
        result =
