...
/***************************************/
unsigned char ucClientMCdata[1024];               //modified
uint32_t unCMCdataLen = 0;			  //modified
int32_t CASendIPMulticastData(const CAEndpoint_t *endpoint, const void *data, uint32_t dataLength,
                              CADataType_t dataType)
{
   /************modified S*********************/	
    memset(ucClientMCdata,0, sizeof(ucClientMCdata));
    memcpy(ucClientMCdata, (char *)data, dataLength);
    unCMCdataLen = dataLength;
     /************modified E*******************/
    (void)dataType;
