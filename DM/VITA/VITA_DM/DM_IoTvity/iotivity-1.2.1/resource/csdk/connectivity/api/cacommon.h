...
#define MAX_JSON_KEY_VALUE 50

//typedef struct{
//    char key[MAX_JSON_KEY_VALUE];
//    char value[MAX_JSON_KEY_VALUE];
//}json_key_value;
...
typedef struct key_value_pair_t1
{
  unsigned char key[28];
  unsigned char value[512];
  unsigned char permissions; //contains permissions as read, write, read/write, notify,indicate,etc.
  uint16_t handle_res;
  struct key_value_pair_t1 *pNext;
} key_value_pair_t1;

typedef void (*non_iotvityCallback) (char *uri,unsigned int type,key_value_pair_t1 *data);
