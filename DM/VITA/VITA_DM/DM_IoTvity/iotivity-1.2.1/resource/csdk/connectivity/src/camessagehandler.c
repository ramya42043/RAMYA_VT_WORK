...
static non_iotvityCallback g_non_iotvityHandler = NULL;
non_iotvityCallback g_handler = NULL;
...
void terminateNonIotivityhandler(void) {
    g_non_iotvityHandler = NULL;
    return ;
}

void setNOnIotivityhandler(void) {
    g_non_iotvityHandler = g_handler;
}
/***********DM FUNCTIONs End**************/
void CASetInterfaceNon_iotvityCallbacks(non_iotvityCallback handler)
{
    g_handler = handler;
    g_non_iotvityHandler = handler;
}
extern uint8_t non_iotivity_add_to_device(uint8_t *);
uint8_t non_iotvityadd_resource(char *resourceName)
{
    //printf("resourcename = %s \n",resourceName);

    if(g_non_iotvityHandler == NULL)
    {
       //printf("Not Initialized");
       return 1;
    }   
    g_non_iotvityHandler(resourceName, 0, NULL);
    return 0;
}

void non_iotvityGet_res_details(char *resourceName, key_value_pair_t1 *data)
{
    ...
    g_non_iotvityHandler(resourceName,1,data);

}
void non_iotvityObserve_res_details(char *resourceName, key_value_pair_t1 *data)
{
    if(g_non_iotvityHandler == NULL)
    {
       // printf("Not Initialized");
    ...
    g_non_iotvityHandler(resourceName,2,data);
