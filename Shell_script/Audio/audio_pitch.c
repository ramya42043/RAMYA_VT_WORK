...
static gint set_pitch(CustomData *data,gfloat i)
{
	gfloat lpitch;
	pitch = i;
	g_object_set(G_OBJECT(data->audiopitch),"pitch",pitch,NULL);
	g_object_get(G_OBJECT(data->audiopitch),"pitch",&lpitch,NULL);
	if(pitch==lpitch)
		return 1;
	else
		return 0;
...
int main(int argc,char *argv[])
{
		...
		if(p == 1)
			g_print("Audio Pitch:Pass\n");
		else
			g_print("Audio pitch:Fail\n");
