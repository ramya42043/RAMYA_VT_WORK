...
typedef struct _CustomData {
		GstElement *pipeline,*source,*audiopitch,*decoder,*conv,*sink,*test;
...
static gint set_rate(CustomData *data,gfloat i)
{
	gfloat lrate;
	rate = i;
	g_object_set(G_OBJECT(data->audiopitch),"rate",rate,NULL);
	g_object_get(G_OBJECT(data->audiopitch),"rate",&lrate,NULL);
	if(rate==lrate)
		return 1;
	else
		return 0;
...
//gint start(gchar *filepath)
{
		...
		gst_bin_add_many(GST_BIN(data.pipeline),data.source,data.decoder,data.audiopitch,data.conv,data.sink,NULL);
		...
		gst_element_link(data.audiopitch,data.sink);
		...
		g_signal_connect(data.audiopitch,"pad-added",G_CALLBACK(on_pad_added),data.sink);
		...
		if( p ==1)
			g_print("Audio Pitch rate:PASS\n");
		else
			g_print("Audio Pitch rate:Fail\n");
