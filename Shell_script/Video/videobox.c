...
static int change_borders(CustomData *data,gint lleft,gint lright,gint ltop,gint lbottom,gint lfill)
{
	...
	//g_print("left :%d right :%d top:%d bottom:%d fill:%d\n",rleft,rright,rtop,rbottom,rfill);
...
//gint start()
{
	...
	//g_print("borders adding and color filling:%d\n",p);
	alarm(10);
	signal(SIGALRM,disable_mainloop);
	g_main_loop_run (data.loop);
	if(p == 1)
		g_print("Videobox:PASS\n");
	else
		g_print("Videobox:FAIL\n");
