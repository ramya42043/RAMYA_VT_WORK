...
static int show_Time(CustomData *data,const char* text,gint flag,guint shade_val,gint horizont,gint vertical,gchar *font,gint resize,guint clr)
{
	...
	/*g_print("text: %s\n",ltext);
	g_print("SHADED BACKGROUND:%d %d\n",shade_background,lflag);
	g_print("SHADED BACKGROUND value:%u %u\n",shade_value,lshade_val);
	g_print("horizontal and vertical  values:%d %d %d %d\n",hval,lhal,vval,lver);
	g_print("font value:%s\n",lfont);
	g_print("Auto resize value:%d %d\n",autoresize,lautoresize);
	g_print("COLOUR value:%u %u\n",color,lclr);*/
...
int main (int argc,char *argv[])
{
	...
	data.pipeline = gst_pipeline_new ("TimeOverlay");
	...
	if(p ==1)
		g_print("TimeOverlay:Pass\n");
	else
		g_print("TimeOverlay:Fail\n");		
	return 0;
