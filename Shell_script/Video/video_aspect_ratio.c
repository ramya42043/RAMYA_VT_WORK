...
int main(int argc,char **argv)
{
	...
	gint p;
	gint i,j;
	for(i=1,j=2;j<15;i++,j++)
	{
	p=change_aspect(&data,i,j);
	sleep(1);
	p=change_aspect(&data,j,i);
	sleep(1);
	}
	alarm(15);
	...
	if(p == 1)
		g_print("Aspect Ratio Crop:PASS\n");
	else
		g_print("Aspect Ratio crop:Fail\n");
